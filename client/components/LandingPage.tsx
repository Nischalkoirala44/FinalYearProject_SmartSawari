...
import { CreditCard, LayoutDashboard, Shield, Zap, ArrowRight, MousePointer2, Play } from "lucide-react";
...
export default function Home() {
      ...
      <section className="relative pt-32 pb-20 px-6 overflow-hidden">
        <div className="container mx-auto max-w-7xl relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            {/* Left: Text Content */}
            <div className="space-y-8 text-left animate-in fade-in slide-in-from-left-8 duration-700">
              ...
              <p className="text-lg text-gray-400 max-w-md font-medium tracking-tight leading-relaxed">
              ...
              <div className="flex flex-wrap gap-4 pt-4">
                <Button asChild size="lg" className="bg-red-600 hover:bg-red-700 text-white text-[10px] font-black uppercase tracking-[0.2em] px-10 py-7 rounded-xl shadow-[0_15px_30px_-10px_rgba(220,38,38,0.5)] transition-all hover:scale-105">
                  <Link href="/vehicles">Explore Fleet</Link>
                </Button>
                <Button asChild variant="outline" className="border-white/10 text-white hover:bg-white/5 text-[10px] font-black uppercase tracking-[0.2em] px-10 py-7 rounded-xl backdrop-blur-xl bg-white/5">
                  <Link href="/owner/register">Partner Portal</Link>
                </Button>
              </div>
            </div>
  ...
  {/* 1. The Base: This ensures it perfectly blends with the previous section */}
  <div className="absolute inset-0 bg-[#0e2233] z-0" />
  
  {/* 2. The Subtle Gradient: Dips into a slightly deeper shade to create "The Floor" */}
  <div className="absolute inset-0 bg-gradient-to-b from-[#0b1a27] via-[#08121a] to-[#0b1a27] opacity-80 z-0" />
  
  {/* 3. Tech Overlay: Very faint horizontal lines (Scanlines) for that automotive screen look */}
  ...
  <div className="container mx-auto max-w-7xl relative z-10">
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
      {[
        ...
        { title: "ENCRYPTED PAY", icon: CreditCard, desc: "Secure Escrow", color: "text-red-600" },
        { title: "FLEET CONTROL", icon: LayoutDashboard, desc: "Owner Terminal", color: "text-white" }
      ].map((feature, i) => (
        <div 
          key={i} 
          className="group p-8 bg-white/[0.02] backdrop-blur-md border border-white/5 rounded-2xl flex flex-col items-center text-center transition-all duration-500 hover:border-red-600/30 hover:bg-white/[0.05]"
        >
          {/* Glowing Icon Base */}
          <div className="relative mb-6">
            ...
            <feature.icon className={`relative w-6 h-6 ${feature.color} transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3`} />
          </div>

          <h3 className="text-[10px] font-black text-white uppercase tracking-[0.4em] mb-2">{feature.title}</h3>
          <p className="text-[9px] text-gray-500 font-bold uppercase tracking-[0.2em]">{feature.desc}</p>
          
          {/* Red Indicator Bar */}
          <div className="w-6 h-[2px] bg-gray-800 mt-6 group-hover:bg-red-600 group-hover:w-12 transition-all duration-500" />
        </div>
      ))}
    </div>
  </div>
      ...
      <section className="py-24 px-6">
        <div className="container mx-auto max-w-7xl">
          <div className="flex justify-between items-end mb-12 border-l-2 border-red-600 pl-6">
            <h2 className="text-3xl font-black text-white uppercase tracking-tighter">Elite <span className="text-red-600">Fleet</span></h2>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="group relative rounded-3xl overflow-hidden bg-[#0e161d] border border-white/5 aspect-[4/5]">
                ...
                <div className="absolute bottom-0 p-8 w-full">
                  ...
                  <h3 className="text-xl font-black text-white uppercase tracking-tighter mb-4">{cat.name}</h3>
      ...
      <section className="py-20 px-6">
        <div className="container mx-auto max-w-5xl">
          <div className="relative overflow-hidden bg-[#BF092F] rounded-[2.5rem] p-10 md:p-16 text-center shadow-2xl">
            ...
            <div className="relative z-10 space-y-6">
              <h2 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tighter leading-none">
                READY TO ESCAPE<br/>THE ORDINARY?
              </h2>
              <p className="text-[10px] md:text-xs font-black text-white/70 uppercase tracking-[0.4em] max-w-lg mx-auto">
                Join the elite community of Smart Sawari drivers today.
              </p>
              
              <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
                <Button asChild className="bg-white text-red-600 hover:bg-gray-100 font-black uppercase tracking-[0.2em] text-[10px] px-8 h-12 rounded-xl">
                  <Link href="/vehicles">Explore Fleet</Link>
                </Button>
                <Button asChild variant="outline" className="border-white/40 text-white hover:bg-white/10 font-black uppercase tracking-[0.2em] text-[10px] px-8 h-12 rounded-xl bg-transparent">
